...
const std::array<SettingDescriptor, kSettingCount> kTable = {{
    ...
    // --- proximity reaction (LTR-553, CoreS3) — all immediate ---------------
    bool_row("prox-enabled",      "prox_en",    ApplyKind::Immediate,
             [](const DeviceConfig& c) -> std::uint32_t { return c.proximity_enabled ? 1 : 0; },
             [](DeviceConfig& c, std::uint32_t v) { c.proximity_enabled = (v != 0); }),
    num_row("prox-near",          "prox_near",  ValueType::U16, ApplyKind::Immediate, 2047, true,
            [](const DeviceConfig& c) -> std::uint32_t { return c.proximity_near; },
            [](DeviceConfig& c, std::uint32_t v) { c.proximity_near = static_cast<std::uint16_t>(v); }),
    num_row("prox-far",           "prox_far",   ValueType::U16, ApplyKind::Immediate, 2047, true,
            [](const DeviceConfig& c) -> std::uint32_t { return c.proximity_far; },
            [](DeviceConfig& c, std::uint32_t v) { c.proximity_far = static_cast<std::uint16_t>(v); }),
    num_row("prox-hold-ms",       "prox_hold",  ValueType::U16, ApplyKind::Immediate, 5000, true,
            [](const DeviceConfig& c) -> std::uint32_t { return c.proximity_hold_ms; },
            [](DeviceConfig& c, std::uint32_t v) { c.proximity_hold_ms = static_cast<std::uint16_t>(v); }),
    num_row("prox-cooldown-s",    "prox_cool",  ValueType::U8, ApplyKind::Immediate, 120, true,
            [](const DeviceConfig& c) -> std::uint32_t { return c.proximity_cooldown_s; },
            [](DeviceConfig& c, std::uint32_t v) { c.proximity_cooldown_s = static_cast<std::uint8_t>(v); }),
    // sensor front-end (調整モード) — indices, see Ltr553Proximity::PsConfig
    num_row("prox-gain",          "prox_gain",  ValueType::U8, ApplyKind::Immediate, 2, true,
            [](const DeviceConfig& c) -> std::uint32_t { return c.proximity_gain; },
            [](DeviceConfig& c, std::uint32_t v) { c.proximity_gain = static_cast<std::uint8_t>(v); }),
    num_row("prox-led-freq",      "prox_lfreq", ValueType::U8, ApplyKind::Immediate, 7, true,
            [](const DeviceConfig& c) -> std::uint32_t { return c.proximity_led_freq; },
            [](DeviceConfig& c, std::uint32_t v) { c.proximity_led_freq = static_cast<std::uint8_t>(v); }),
    num_row("prox-led-duty",      "prox_lduty", ValueType::U8, ApplyKind::Immediate, 3, true,
            [](const DeviceConfig& c) -> std::uint32_t { return c.proximity_led_duty; },
            [](DeviceConfig& c, std::uint32_t v) { c.proximity_led_duty = static_cast<std::uint8_t>(v); }),
    num_row("prox-led-current",   "prox_lcur",  ValueType::U8, ApplyKind::Immediate, 4, true,
            [](const DeviceConfig& c) -> std::uint32_t { return c.proximity_led_current; },
            [](DeviceConfig& c, std::uint32_t v) { c.proximity_led_current = static_cast<std::uint8_t>(v); }),
    num_row("prox-pulses",        "prox_pulse", ValueType::U8, ApplyKind::Immediate, 15, true,
            [](const DeviceConfig& c) -> std::uint32_t { return c.proximity_pulses; },
            [](DeviceConfig& c, std::uint32_t v) { c.proximity_pulses = static_cast<std::uint8_t>(v); }),
    num_row("prox-meas-rate",     "prox_rate",  ValueType::U8, ApplyKind::Immediate, 7, true,
            [](const DeviceConfig& c) -> std::uint32_t { return c.proximity_meas_rate; },
            [](DeviceConfig& c, std::uint32_t v) { c.proximity_meas_rate = static_cast<std::uint8_t>(v); }),
    num_row("prox-offset",        "prox_off",   ValueType::U16, ApplyKind::Immediate, 1023, true,
            [](const DeviceConfig& c) -> std::uint32_t { return c.proximity_offset; },
            [](DeviceConfig& c, std::uint32_t v) { c.proximity_offset = static_cast<std::uint16_t>(v); }),
