...
//   - 単位連結 / フォルマントや分割不要の短文は、全体が 1 チャンクで渡される。
//   - sanoTTS は句読点 (、。) ごとに 1 句ずつ合成して渡す。句と句の間には HMM の pau と
//     同じ長さの無音を明示的に挟む (句の後ろに足すので、チャンクは「句 + 間」)。
//     synthesize_ex と同様に 22.05 kHz で出力する (SynthChunk::sample_rate)。
